...
async function criarStickerTexto(texto) {
  ...
  const textosSvg = linhas.map((linha, i) => {
    ...
    return `<text x="256" y="${y}" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="bold" fill="#000000" text-anchor="middle">${escaparXml(linha)}</text>`
  }).join('\n')
...
async function criarStickerFF(sock, jid, participante, textoCitado) {
  ...
  linhas.forEach((linha, index) => {
    ...
    textoSvg += `<text x="${bolhaX + paddingX}" y="${y}" font-family="Arial, Helvetica, sans-serif" font-size="25" font-weight="400" fill="#e9edef">${escaparXml(linha)}</text>`
  })
  ...
  const svgBolha = `
    <svg width="${CANVAS}" height="${CANVAS}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${bolhaX + paddingX}" y="${bolhaY + 30}" font-family="Arial, Helvetica, sans-serif" font-size="20" font-weight="700" fill="#53bdeb">${escaparXml(nomeFinal)}</text>
      ${textoSvg}
      <text x="${horarioX}" y="${horarioY}" font-family="Arial, Helvetica, sans-serif" font-size="15" fill="#8696a0">${horario}</text>
      <text x="${horarioX + 30}" y="${horarioY}" font-family="Arial, Helvetica, sans-serif" font-size="15" fill="#53bdeb">✓✓</text>
    </svg>
  `
...
async function processarFigurinha(sock, jid, msg, texto) {
  ...
  // CORREÇÃO CRUCIAL AQUI: pega o participante correto da mensagem citada
